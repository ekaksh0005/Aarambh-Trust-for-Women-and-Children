...
class WP_Optimization_images extends WP_Optimization {
	...
	private function _tasks_queue() {
		return WP_Optimize_Tasks_Queue::this('images_optimization');
	}
	...
	public function after_optimize() {
		...
		// clear cached value for output.
		$this->delete_last_output();
		...
		foreach ($this->blogs_ids as $blog_id) {
			...
			if (!empty($unused_posts_images)) {
				foreach ($unused_posts_images as $id => $image) {
					...
					$unused_images[$blog_id][] = array(
						'id' => $id,
						'url' => isset($image['url']) ? utf8_encode($image['url']) : ''
					);
				}
			}
	...
	public function after_get_info() {
		...
		if (isset($this->data['support_ajax_get_info']) && !$this->is_debug_mode()) {
			...
			if ($this->_tasks_queue()->is_empty()) {
				...
				$this->build_get_info_output();
				$this->save_last_output();
			} else {
	...
	private function get_images_from_options() {
		...
		$option_values = $wpdb->get_col($wpdb->prepare("SELECT option_value FROM {$wpdb->options} WHERE option_name NOT REGEXP %s AND option_value REGEXP %s", '^_', '[0-9]{4}/[0-9]{2}/[^\\\/\'\"]+.('.join('|', $this->_images_extensions).')'));
	...
	public function process_get_posts_images_result($result) {
		...
		if ($result['processed'] < $result['limit']) {
			...
			$this->save_to_cache('unused_posts_images_ids', $unused_images_ids, $blog_id);

			$this->_tasks_queue()->add_task(new WP_Optimize_Queue_Task(array(get_class($this), 'task_calculate_unused_posts_images_size'), array($blog_id), '', $this->calc_priority($blog_id, 6)));
	...
	/**
	 * Gets images information for images found in posts.
	 *
	 * @param int $blog_id
	 */
	public function task_calculate_unused_posts_images_size($blog_id) {

		$this->log('task_calculate_unused_posts_images_size({blog_id})', array('blog_id' => $blog_id));

		$this->message(__('Calculating size...', 'wp-optimize'), $blog_id);

		$unused_images_ids = $this->get_from_cache('unused_posts_images_ids', $blog_id);

		$posts_images = array();

		$this->switch_to_blog($blog_id);

		if (!empty($unused_images_ids)) {
			foreach ($unused_images_ids as $i => $image_id) {
				...
				if (!$this->is_attachment_metadata_loaded($image_id)) {
					$this->preload_attachments_metadata($unused_images_ids, $i);
				}

				$posts_images[$image_id] = $this->get_attachment_info($image_id, false);
			}
		}

		$this->restore_current_blog();

		$this->delete_from_cache('unused_posts_images_ids', $blog_id);
		// save results to cache.
		$this->save_to_cache('unused_posts_images', $posts_images, $blog_id);
	}
	...
	public function task_get_unused_images_files($blog_id = 1) {
		...
		$sub_dirs = $this->get_upload_sub_dirs($blog_id);

		if (!empty($sub_dirs)) {
			foreach ($sub_dirs as $sub_dir) {
				$this->_tasks_queue()->add_task(new WP_Optimize_Queue_Task(array(get_class($this), 'get_unused_images_in_sub_directory'), array($sub_dir, $blog_id), array(get_class($this), 'process_get_unused_images_in_sub_directory_result'), $this->calc_priority($blog_id, 11)));
			}
		}
	...
	/**
	 * Returns unused image files in sub directory.
	 *
	 * @param string $sub_directory upload sub directory.
	 * @param int    $blog_id
	 * @return array
	 */
	public function get_unused_images_in_sub_directory($sub_directory, $blog_id = 1) {
		$unused_images = $_image_files = array();

		$this->log('get_unused_images_in_sub_directory({sub_directory}, {blog_id})', array('sub_directory' => $sub_directory, 'blog_id' => $blog_id));
	 ...
	 * @param array $result
	 */
	public function process_get_unused_images_in_sub_directory_result($result) {
		$blog_id = $result['blog_id'];

		$this->log('process_get_unused_images_in_sub_directory_result({blog_id})', array('blog_id' => $blog_id));
	...
	public function process_get_all_image_sizes_results($result) {
		...
		if (!empty($result['image_ids'])) {
			...
			foreach ($result['image_ids'] as $i => $image_id) {

				// if data for attachment is not loaded from database then preload data for the next portion.
				if (!$this->is_attachment_metadata_loaded($image_id)) {
					$this->preload_attachments_metadata($result['image_ids'], $i);
				}
		...
		if (!empty($blog_ids)) {
			foreach ($blog_ids as $blog_id) {
				...
				if ($remove_all_images) {
					...
					if (!empty($unused_posts_images)) {
						foreach (array_keys($unused_posts_images) as $image_id) {
							$attachment_info = $this->get_attachment_info($image_id);
							$is_image_removed = $this->remove_attachment($image_id);
							// update information about sizes in cache.
							$this->remove_sizes_info($all_image_sizes, $attachment_info);

							$removed['files']++;
							$removed[$blog_id]++;
							$removed['size'] += $unused_posts_images[$image_id]['size'];

							unset($unused_posts_images[$image_id]);
						}
					}
					...
					if (array_key_exists($blog_id, $images)) {
						...
						foreach ($images[$blog_id] as $image) {
							if (is_numeric($image)) {
								...
								$is_image_removed = $this->remove_attachment($image);
								// update information about sizes in cache.
								$this->remove_sizes_info($all_image_sizes, $attachment_info);

								$removed['files']++;
								$removed[$blog_id]++;
								$removed['size'] += $unused_posts_images[$image]['size'];

								unset($unused_posts_images[$image]);
							} else {
								// if posted url then remove file from upload directory.
								$is_image_removed = $this->remove_file($base_upload_dir.'/'.html_entity_decode($image));
	...
	private function remove_sizes_info(&$sizes_info, $image_info) {
		if (empty($image_info['sizes'])) return;
		...
		do {
			if ($one_iteration) {
				$query = $wpdb->prepare(
					...
					'attachment',
					'image/%',
					// get only attachments matched with upload path.
					'[0-9]{4}/[0-9]{2}/[^\\\/\'\"]+.('.join('|', $this->_images_extensions).')'
				);
			} else {
				$query = $wpdb->prepare(
					"SELECT p.ID FROM {$wpdb->posts} p ".
					...
					" ON p.ID = pm.post_id AND pm.meta_key = '_wp_attached_file' ".
					"WHERE p.post_type=%s AND p.post_mime_type LIKE %s ".
					" AND (pm.meta_value REGEXP %s) LIMIT %d, %d;",
					'attachment',
					'image/%',
					// get only attachments matched with upload path.
					'[0-9]{4}/[0-9]{2}/[^\\\/\'\"]+.('.join('|', $this->_images_extensions).')',
					...
					foreach ($ids as $id) {
						...
						if (!empty($meta) && !empty($meta['sizes'])) {
							...
							if ($updated) {
								...
								// if removed unused image then update cached data.
								if (array_key_exists($id, $unused_posts_images)) {
									$unused_posts_images[$id] = $this->get_attachment_info($id, false);
								}
							}
						}
					}
	...
	public function get_attachment_info($attachment_id, $extended = true) {
		$attachment_info = array('files' => 0, 'size' => 0);
		$base_upload_dir = $this->get_upload_base_dir();
		...
		if ($meta && preg_match('/^\d{4}\/\d{2}/', $meta['file'], $sub_dir)) {
			$sub_dir = $sub_dir[0];
	...
	/**
	 * Save current output to cache.
	 */
	public function save_last_output() {
		$to_save = array(
			'blogs_ids' => $this->blogs_ids,
			'output' => $this->get_results()->output
		);

		$this->save_to_cache('last_output', $to_save);
	}

	/**
	 * Check if exists last generated output in cache and return it.
	 *
	 * @return bool
	 */
	public function get_last_output() {
		$cached = $this->get_from_cache('last_output');

		if (empty($cached) || !is_array($cached) || $this->is_debug_mode()) return false;

		$cached_blogs_ids = $cached['blogs_ids'];
		$current_blogs_ids = $this->blogs_ids;

		$diff1 = array_diff($current_blogs_ids, $cached_blogs_ids);
		$diff2 = array_diff($cached_blogs_ids, $current_blogs_ids);

		// if blogs_ids equal then return cached output.
	...
	/**
	 * Delete from cache last output information.
	 */
	public function delete_last_output() {
		$this->delete_from_cache('last_output');
	}
	...
	private function clear_cached_data() {
		...
		if ($this->is_multisite_mode()) {
			$keys = $wpdb->get_col("SELECT meta_key FROM {$wpdb->sitemeta} WHERE meta_key LIKE '%wpo_images_cache_%'");
		} else {
			$keys = $wpdb->get_col("SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE '%wpo_images_cache_%'");
		}
	...
	private function parse_images_in_content($content) {
		preg_match_all('/\d{4}\/\d{2}\/[^\\\'\"]+\.('.join('|', $this->_images_extensions).')/Ui', $content, $images);
	 ...
	 * @param int   $offset
	 */
	private function preload_attachments_metadata(&$attachment_ids, $offset = 0) {
		global $wpdb;

		$this->log('preload_attachments_metadata()');

		$item_size = 1024 * 50; // ~5-10kb is a memory size used by one attachment record, we get 5x to be safe with memory limit.
		...
		$can_loaded = floor(WP_Optimize()->get_free_memory() / $item_size);
		...
		if (count($attachment_ids) <= $can_loaded) {
			...
			$metadata = $wpdb->get_results("SELECT `post_id` as `id`, `meta_value` FROM {$wpdb->postmeta} WHERE (`meta_key` = '_wp_attachment_metadata') AND `post_id` IN ('" . join("','", array_splice($attachment_ids, $offset, $can_loaded)) . "')", ARRAY_A);
		}
	 ...
	 *   task_calculate_unused_posts_images_size - 6
