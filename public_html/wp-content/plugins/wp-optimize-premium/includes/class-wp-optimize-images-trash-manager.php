...
class WP_Optimize_Images_Trash_Manager extends Updraft_Task_Manager_1_2 {
	...
	private function get_all_image_files($image_id) {
		...
		// delete basedir part to keep only relative pat part
		$src = str_replace($upload_dir['basedir'], '', $src);

		$path_info = pathinfo($src);

		// search all resized files for provided image
		$list = glob($upload_dir['basedir'].$path_info['dirname'].'/'.$path_info['filename'].'-*');
		$list[] = $src;

		// delete basedir part for found image files
		foreach ($list as $i => $filename) {
			$list[$i] = str_replace($upload_dir['basedir'], '', $filename);
		}
