...
class WP_Optimize_Premium {
	...
	public function __construct() {
		...
		add_action('wpo_tables_list_additional_column_data', array($this, 'tables_list_additional_column_data'), 10, 2);
		...
		add_action('wpo_premium_scripts_styles', array($this, 'enqueue_scripts_styles'));
		...
		try {
			new Updraft_Manager_Updater_1_8('https://getwpo.com/plugin-info/', 1, 'wp-optimize-premium/wp-optimize.php');
		} catch (Exception $e) {
	...
	public function enqueue_scripts_styles() {
		...
		wp_enqueue_script('wp-optimize-premium-js', WPO_PLUGIN_URL . 'js/wpo-premium' . $min_or_not_internal . '.js', array('jquery', 'jquery-ui-dialog', 'wp-optimize-admin-js', 'wp-optimize-images-view-js', 'jquery-ui-datepicker', 'select2'), $enqueue_version);
	 ...
	 *
	 * @param  array $table_information An array of information about the table
	 */
	public function wpo_tables_list_before($table_information) {
		if (false === $table_information['is_optimizable'] && $table_information['inno_db_tables'] > 0) {
			?>
			<p class="margin-one-percent">
	...
	public function include_lazy_load() {
		// Avoid conflicts if something else is already doing lazy-loading
		if (count($this->lazyload_already_provided_by()) > 0) return;
