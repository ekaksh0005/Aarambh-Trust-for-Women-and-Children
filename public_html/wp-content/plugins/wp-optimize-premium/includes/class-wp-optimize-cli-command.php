...
class WP_Optimize_CLI_Command extends WP_CLI_Command {
	...
	public function __invoke($args, $assoc_args) { // phpcs:ignore PHPCompatibility.FunctionNameRestrictions.NewMagicMethods.__invokeFound
		...
		$commands = array(
			...
			'cache enable' => __('Enable page cache', 'wp-optimize'),
			'cache disable' => __('Disable page cache', 'wp-optimize'),
			'cache purge' => __('Purge contents from cache', 'wp-optimize'),
			'cache preload' => __('Preload contents into cache', 'wp-optimize'),
			'cache status' => __('Get current cache status', 'wp-optimize'),
		);
